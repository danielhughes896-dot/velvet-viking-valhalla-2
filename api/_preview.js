...
function validate(body){
  ...
  /* LIMITS.volume[0] is a floor to EXCEED, not to reach -- the app's own
     ...
     READ BEFORE THE EVENT BRANCH, because whether this request is a race block
     at all depends on it -- see the entry boundary below. */
  ...
  /* ---------- THE MINIMUM RUNNING FOUNDATION FOR A RACE GOAL ----------
     THE SAME BOUNDARY THE APP ENFORCES, FROM THE SAME NUMBER, and it ROUTES
     rather than refusing. Below it the athlete does not yet demonstrate enough
     established running for Race Goal methodology to express differentiated
     preparation, so the block this endpoint previews is an Aerobic Base block
     -- a real one, generated by the same engine, on the distance they asked
     about. `routedFrom` says so, so the preview can name what it is showing
     and no surface has to infer it.

     Enforced here as well as in the app because /start builds through this
     endpoint before an athlete has an account: a rule the app holds and the
     pre-auth surface does not is not a rule. */
  const raceEntryMin = BUILDER_SPEC.validation.raceGoalMinWeeklyKm;
  const belowRaceEntry = purpose === 'race' && isFinite(raceEntryMin) &&
                         volume + 1e-9 < raceEntryMin;
  const effectivePurpose = belowRaceEntry ? 'base' : purpose;

  const hasEvent = effectivePurpose === 'race' && b.hasEvent === true;
  ...
  } else if (belowRaceEntry){
    /* A routed request may have arrived with an event and no block length at
       all. The routed block gets this purpose's own default, which is the same
       number the app's builder puts in the field when the purpose changes. */
    weeks = parseInt(b.weeks, 10);
    if (!isFinite(weeks) || weeks < wRange[0] || weeks > wRange[1])
      weeks = defaultWeeksFor('base');
  } else {
    weeks = parseInt(b.weeks, 10);
    if (!isFinite(weeks) || weeks < wRange[0] || weeks > wRange[1])
  ...
  const shape = PURPOSE_SHAPE[effectivePurpose];
  const buildDistance = shape.forceDistance || distanceKey;
  const buildVolume = Math.round(volume * shape.volumeFactor * 10) / 10;

  return { ok: true, input: {
    purpose: effectivePurpose,
    /* Present only when the request was routed, so a response for a request
       that was not carries exactly the shape it always did. */
    routedFrom: belowRaceEntry ? 'race' : undefined,
    routedReason: belowRaceEntry ? 'below_race_goal_entry_volume' : undefined,
    raceGoalMinWeeklyKm: belowRaceEntry ? raceEntryMin : undefined,
