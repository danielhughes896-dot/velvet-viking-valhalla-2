...
function athlete(pinned) {
  ...
  /* TWO QUALITY SESSIONS A WEEK, because the two dates below are properties of
     that week's shape. Quality frequency is earned from the athlete's own
     logged response rather than granted by the day count, so a fixture that
     says nothing about the athlete gets one a week -- which moves the quality
     day, which moves which rest day is "before a key session", which moves
     where supportForWeek() places the work. The rule under test is unchanged
     and so are the dates; the fixture now describes the athlete whose week
     those dates belong to. */
...
test('no prescription or session-selection output changed', () => {
  ...
  /* WHAT THIS ASSERTS, AND WHY IT IS NO LONGER A FROZEN TOTAL. It used to pin
     the four reference plans at 39 supporting sessions, on the reasoning that
     any movement meant the engine had moved and the completion branch was not
     allowed to move it. That reasoning was right about the completion path and
     wrong as a test: the plan-mathematics programme IS allowed to move the
     engine, and did -- quality frequency is now earned rather than granted by
     the day count, which moves the quality day, which moves which rest days
     are "before a key session", which moves where supportForWeek() places the
     work. The total is 41 on this engine.

     The invariant the title claims is narrower and is now measured directly:
     COMPLETING supporting work changes neither the supporting-work selection
     nor any prescription. That cannot drift with the engine, and it is the
     thing the completion path could actually break. */
  let items = 0;
  for (const v of variants) {
    ...
    const selection = app => weeks.map(w => (app.supportForWeek(w) || [])
      .map(x => x.kind + '@' + x.dayId).join(',')).join('|');
    /* Which days carry supporting work, without which kind lands on them. */
    const placement = app => weeks.map(w => (app.supportForWeek(w) || [])
      .map(x => x.dayId).join(',')).join('|');
    const prescriptions = app => app.state.days.map(d => d.date + ':' + d.type + ':' + d.km + ':' +
      ((d.prescription || {}).archetype || '-') + ':' +
      JSON.stringify((d.prescription || {}).params || null)).join('\n');

    const selBefore = selection(a), presBefore = prescriptions(a), placeBefore = placement(a);
    for (const w of weeks) items += (a.supportForWeek(w) || []).length;

    /* Complete one, on the day it is prescribed for, through the real path. */
    const target = weeks.map(w => (a.supportForWeek(w) || [])[0]).filter(Boolean)[0];
    if (target){
      const on = at(target.dayId);
      on.state = JSON.parse(JSON.stringify(a.state));
      on.handleSupportDone(target.dayId);
      const done = on.state.days.filter(d => d.id === target.dayId)[0];
      assert.ok(done.support && done.support.completedAt, 'the fixture really completed it');
      assert.equal(prescriptions(on), presBefore,
        v.distanceKey + ': completing supporting work moved a prescription');
      /* PLACEMENT IS INVARIANT, KIND IS NOT, AND THAT ASYMMETRY IS THE DESIGN.
         supportForWeek() reads the last completed kind out of
         supportRecentCompletions(14) and prefers a different one next --
         "variety, and only where it costs nothing". So logging a session is
         SUPPOSED to rotate the kind that comes after it. What may not move is
         where the work sits: completing on Monday must not add, remove, or
         relocate the week's supporting days. Asserting the whole selection
         string would have demanded the rotation not happen, which is the
         opposite of the rule. */
      assert.equal(placement(on), placeBefore,
        v.distanceKey + ': completing supporting work moved which days carry it');
      const firstAfter = (on.supportForWeek(dayOf(on, target.dayId).week) || [])[0];
      if (firstAfter) assert.notEqual(firstAfter.kind, target.kind,
        v.distanceKey + ': rotation did not respond to the completed kind');
    }
    assert.equal(selection(a), selBefore,
      v.distanceKey + ': reading the selection twice must not change it');
  ...
  assert.equal(items, 41,
    'the four reference plans prescribe 41 supporting sessions in total; if this moves, '
