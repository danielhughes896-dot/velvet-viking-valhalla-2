...
/* Two devices built this way are byte-identical by construction, which is what
   ...
   THE ATHLETE EARNS TWO QUALITY SESSIONS A WEEK because these tests need a
   supporting-work companion ON TODAY -- completion is only permitted there --
   and supportForWeek() places work relative to the week's key sessions. A
   fixture that says nothing about the athlete now gets one quality session a
   week, which moves the key day, which moves which rest day is "before a key
   session", which moves the supporting day off 2026-09-02. Nothing about the
   signature contract under test changes; the fixture now describes the athlete
   whose week carries the companion these tests complete and decline. */
function device() {
  const a = app();
