...
test('the real generator agrees: the 6km athlete is never coaching-suspicious for it', () => {
  /* AT A DISTANCE WHERE A 6km/WEEK ATHLETE STILL BUILDS ONE. The marathon, the
     half and now 5K/10K too (the continuation of this same correction) route
     an athlete this far below their pathway's entry week to a foundation
     block, and the race block they would otherwise have been given opens at
     the pathway's entry -- so there is no 6 -> 7 transition in any of their
     plans to ask about any more. Ultra has no dedicated pathway and still
     builds from the athlete's own figure, which is where the property under
     test -- a one-kilometre step is descriptively over 10% and is not
     coaching-suspicious for it -- actually lives. Same numbers, same
     assertion. */
  const c = auditCase({ distanceKey: 'ultra', volume: 6, weeks: 16, scheduleKey: 'd5' });
