...
const BUILDER_SUBSET = files(['builderNineStages','cherryLacquer','healthDataConsent',
                             'betaLegalDiscoverability','yearRoundLifecycle','themeSystem']);
...
const CASES = [
  ...
  ['accent: dark mode ships the raw brand value and loses its control boundary',
   'protected/velvet-viking-valhalla.html',
   "    --cherry:#9A546C;                   /* bare indicators -- 3.01:1 on --bg-3 */",
  ...
  ['accent: the accent wanders onto the Threshold workout colour',
   'protected/velvet-viking-valhalla.html',
   "    --cherry:#9A546C;                   /* bare indicators -- 3.01:1 on --bg-3 */",
   "    --cherry:#AD4A5F;                   /* bare indicators -- 3.01:1 on --bg-3 */", ACCENT_SUBSET],
  ['accent: a workout type is repainted in the brand colour',
   'protected/velvet-viking-valhalla.html',
   "    --c-threshold:#B5505A; --c-threshold-soft:rgba(181,80,90,0.16);",
   "    --c-threshold:#9A546C; --c-threshold-soft:rgba(181,80,90,0.16);", ACCENT_SUBSET],
  ['accent: two workout types collapse into the same colour',
   'protected/velvet-viking-valhalla.html',
   "    --c-tempo:#CC9245; --c-tempo-soft:rgba(204,146,69,0.16);",
   "    --c-tempo:#B5505A; --c-tempo-soft:rgba(204,146,69,0.16);", ACCENT_SUBSET],
  // ---- ONE CONTINUOUS CREAM, AND THE PERMISSION'S NEW HOME ----
  ['surface: the tinted card comes back in light', 'protected/velvet-viking-valhalla.html',
   "    --surface-2:var(--bg); --surface-3:var(--bg); --surface-4:var(--bg);",
   "    --surface-2:var(--bg-2); --surface-3:var(--bg-3); --surface-4:var(--bg-4);", BUILDER_SUBSET],
  ['surface: the flattening leaks into dark', 'protected/velvet-viking-valhalla.html',
   "    --surface-2:var(--bg-2);\n    --surface-3:var(--bg-3);\n    --surface-4:var(--bg-4);",
   "    --surface-2:var(--bg);\n    --surface-3:var(--bg);\n    --surface-4:var(--bg);", BUILDER_SUBSET],
  ['surface: a state fill is flattened along with the decoration',
   'protected/velvet-viking-valhalla.html',
   "  .day:hover{background:var(--bg-4);}",
   "  .day:hover{background:var(--surface-4);}", BUILDER_SUBSET],
  ['surface: the day row loses the border that replaced its fill',
   'protected/velvet-viking-valhalla.html',
  ...
  ['consent: Review stops naming it as optional', 'protected/velvet-viking-valhalla.html',
   "'<div class=\"setup-section-title\" style=\"margin-top:0;\">Optional permission</div>'+",
   "'<div class=\"setup-section-title\" style=\"margin-top:0;\">Health data</div>'+", BUILDER_SUBSET],
  // ---- THE ANTI-ROSE RULE ----
