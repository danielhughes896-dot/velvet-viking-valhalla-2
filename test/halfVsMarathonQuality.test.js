...
test('CULMINATION — the half ends on threshold and the marathon ends on goal pace', () => {
  /* What each block's LAST developed quality session is. A half marathon is
     ...
     HQ NARROW PATHWAY CORRECTION -- KNOWN, NARROW EXCEPTION, ONCE: Experienced
     Half briefly landed on goal_pace_block instead of threshold_continuous.
     TEMPO_STRUCTURE_POOL.Peak is [structGoalPaceBlock,
     ...
     tempo occurrences preceded it, not by distance -- the same class of
     occurrence-count sensitivity documented elsewhere this session as
     fragile to touch directly (see the TEMPO test below and its
     soloOccQ/soloOccT comment in buildBlockWeeks).

     HQ DAY-COUNT/START-VOLUME CORRECTION -- the day-count/support-day
     arithmetic moved again and flipped the parity back: Experienced Half now
     culminates on threshold_continuous, the same as every other pathway.
     The exception is retired rather than kept as a live branch nobody
     exercises; if a future pathway change flips this parity again it should
     be re-added narrowly, the way it was the first time. */
  EXPS.forEach(e => {
    const h = sessions('half', e), f = sessions('full', e);
